#!/usr/bin/env python3
"""
EY AI & Data Challenge 2026 - Water Quality Prediction v21.0
REFINED MODEL - Addressing Geographic Mismatch

Key insights:
- 0 overlapping locations between train and validation
- Validation is 100% in Eastern Cape, train has only 2 locations there
- Nearest train-to-val distance: ~90km (0.8 degrees)
- loc_target_* features are USELESS for validation (all NaN)

Strategy:
1. NO location-specific target encoding (useless for new locations)
2. Environmental IDW: weight by environmental similarity, not just distance
3. Robust environmental features that transfer across geography
4. Target-specific Optuna tuning with spatial CV
5. Stacking + weighted blending with IDW
6. Calibration to Eastern Cape distributions
"""
...
# =============================================================================
# FEATURE ENGINEERING - No location-specific target leakage
# =============================================================================

def create_features(wq_df, ls_df, tc_df, climate_df,
                     train_wq=None, target=None,
                     elev_cache=None,
                     train_tc=None, train_climate=None, train_ls=None):
    """
    Create features WITHOUT location-specific target encoding.
    Focus on environmental features that generalize.
    train_tc/train_climate/train_ls: training data needed for env IDW.
    """
    df = pd.DataFrame()
    eps = 1e-10
    n = len(wq_df)
    ...
    # Elevation-terrain interactions
    ...
    df['is_autumn'] = np.isin(month, [3, 4, 5]).astype(float)
    df['is_winter'] = np.isin(month, [6, 7, 8]).astype(float)
    df['is_spring'] = np.isin(month, [9, 10, 11]).astype(float)
    df['is_wet_season'] = np.isin(month, [10, 11, 12, 1, 2, 3]).astype(float)
    df['year_norm'] = (year - 2011) / 4.0

    # =========================================================================
    # D. SPECTRAL (Comprehensive)
    # =========================================================================
    ...
    if 'precip_30d' in df.columns:
        ...
        df['wet_precip'] = df['is_wet_season'] * df['precip_30d']

    # =========================================================================
    # G. IDW FEATURES (Geographic + Environmental)
    # =========================================================================
    if train_wq is not None and target is not None:
        _add_idw_features(df, lat, lon, train_wq, target, elev_cache,
                          ...
                          train_tc=train_tc, train_climate=train_climate,
                          train_ls=train_ls)

    return df


def _build_environmental_vector(lat, lon, elev, pet, precip, temp, ndmi, turbidity):
    """Build normalized environmental vector for similarity matching."""
    return np.array([
        lat / 10.0,           # Scale to ~[-3.5, -2.2]
        lon / 10.0,           # Scale to ~[1.8, 3.2]
        elev / 1000.0,        # Scale to ~[0, 2.5]
        pet / 200.0,          # Scale to ~[0.5, 1.0]
        precip / 100.0,       # Scale to ~[0, 5]
        temp / 30.0,          # Scale to ~[0.1, 1.0]
        ndmi,                 # Already ~[-1, 1]
        turbidity,            # Already ~[0, 3]
    ])
...
def _add_idw_features(df, lat, lon, train_wq, target, elev_cache,
                       ls_df, tc_df, climate_df,
                       train_tc=None, train_climate=None, train_ls=None):
    """Add both geographic and environmentally-weighted IDW."""
    eps = 1e-10
    ...
    for k_nn in [3, 5, 10]:
        k_actual = min(k_nn, len(train_coords))
        ...
        df[f'nn{k_nn}_median'] = np.median(train_values[idxs_k], axis=1)

    # === Environmental IDW ===
    # Build environmental vectors for ALL training points using training data
    ...
    # Use TRAINING terraclimate/climate data (not current dataset's)
    if train_tc is not None and len(train_tc) == train_n:
        train_pet = train_tc['pet'].values.astype(float)
    else:
        train_pet = tc_df['pet'].values[:train_n] if len(tc_df) >= train_n else np.zeros(train_n)

    val_pet = tc_df['pet'].values.astype(float)

    # Training Landsat data for environmental features
    if train_ls is not None and len(train_ls) == train_n:
        train_ndmi = train_ls['NDMI'].values.astype(float)
        train_nir = train_ls['nir'].values.astype(float)
        train_green = train_ls['green'].values.astype(float)
        train_turb = train_nir / (train_green + eps)
    else:
        train_ndmi = np.zeros(train_n)
        train_turb = np.zeros(train_n)

    val_ndmi = ls_df['NDMI'].values.astype(float)
    val_turb = ls_df['nir'].values.astype(float) / (ls_df['green'].values.astype(float) + eps)

    # Training climate features
    if train_climate is not None and len(train_climate) == train_n:
        train_precip = train_climate['precip_30d'].values.astype(float)
        train_temp = train_climate['temp_mean'].values.astype(float)
        train_temp_range = train_climate['temp_range'].values.astype(float)
    else:
        train_precip = np.zeros(train_n)
        train_temp = np.zeros(train_n)
        train_temp_range = np.zeros(train_n)

    # Val climate features
    if climate_df is not None and len(climate_df) == len(lat):
        val_precip = climate_df['precip_30d'].values.astype(float)
        val_temp = climate_df['temp_mean'].values.astype(float)
        val_temp_range = climate_df['temp_range'].values.astype(float)
    else:
        val_precip = np.zeros(len(lat))
        val_temp = np.zeros(len(lat))
        val_temp_range = np.zeros(len(lat))

    # Build environmental feature matrices with MULTIPLE features
    ...
    # Replace NaN before normalization
    ...
    for k_env in [10, 30, 50]:
        ...
        weights = 1.0 / env_d
        weights = weights / weights.sum(axis=1, keepdims=True)
        vals_selected = np.array([train_values[env_idxs[i]] for i in range(len(lat))])
        df[f'env_idw_k{k_env}'] = np.sum(weights * vals_selected, axis=1)

        # Env IDW std
        env_mean_pred = df[f'env_idw_k{k_env}'].values
        df[f'env_idw_k{k_env}_std'] = np.sqrt(
            np.sum(weights * (vals_selected - env_mean_pred[:, None])**2, axis=1)
        )
    ...
    for alpha in [0.3, 0.5, 0.7]:
        # Get env distances for the geographically nearest points
        env_d_geo = np.array([
            env_dists[i, geo_idxs_all[i]] for i in range(len(lat))
        ])
    ...
    # IDW agreement features
    df['idw_range'] = df.get('idw_k5_p10', df.get('idw_k5_p20', pd.Series(0))) - df.get('idw_k50_p10', pd.Series(0))
...
def optimize_xgboost(X_train, y_train, groups, n_trials=80):
    def objective(trial):
        params = {
...
def optimize_lightgbm(X_train, y_train, groups, n_trials=80):
    def objective(trial):
        params = {
            'n_estimators': trial.suggest_int('n_estimators', 100, 600),
...
def train_stacking_ensemble(X_train, y_train, X_val, groups,
    ...
    """
    Level 1: XGBoost, LightGBM, ExtraTrees, GradientBoosting, RandomForest
    Level 2: Ridge meta-learner on OOF predictions
    """
    n_train = len(X_train)
    kf = GroupKFold(n_splits=5)
    ...
    def train_base_model(ModelClass, params, seed):
        ...
        for train_idx, val_idx in folds:
            ...
            if 'random_state' in params_copy or hasattr(ModelClass, 'random_state'):
    ...
    # Evaluate individual model OOF scores
    print('     Individual OOF R² scores:')
    ...
    oof_scores = [r2_score(y_train, meta_train[i]) for i in range(len(meta_train))]
    # Only positive weights
    oof_scores_pos = np.maximum(oof_scores, 0)
    if oof_scores_pos.sum() > 0:
        oof_weights = oof_scores_pos / oof_scores_pos.sum()
        weighted_avg = np.sum([w * v for w, v in zip(oof_weights, meta_val)], axis=0)
    else:
        weighted_avg = simple_avg

    return stacking_pred, simple_avg, weighted_avg, meta_model.coef_
...
def select_features(X_train, y_train, feature_names, groups, top_n=100):
    ...
    # Keep features with non-zero importance, up to top_n
    nonzero_features = feature_imp[feature_imp['importance'] > 0]
    top_features = nonzero_features.head(top_n)['feature'].values
...
def main():
    print('=' * 70)
    print('EY Water Quality v21.0 - Refined (No Location Leakage)')
    print('=' * 70)
    ...
    all_predictions = {}  # target -> dict of prediction types

    for target in TARGET_COLS:
        print(f'\n{"="*70}')
        print(f'[2/6] Processing: {target}')
        print(f'{"="*70}')

        cfg = target_configs[target]

        # Feature Engineering
        print('\n  [a] Creating features (no location-target leakage)...')
        ...
        # Optuna Tuning
        print('  [d] Optuna hyperparameter optimization (80 trials each)...')
        print('     Optimizing XGBoost...')
        xgb_best = optimize_xgboost(X_train_sel, y_train, location_groups, n_trials=80)
        print(f'     XGBoost: depth={xgb_best["max_depth"]}, '
              f'lr={xgb_best["learning_rate"]:.4f}, n_est={xgb_best["n_estimators"]}')

        print('     Optimizing LightGBM...')
        lgb_best = optimize_lightgbm(X_train_sel, y_train, location_groups, n_trials=80)
        print(f'     LightGBM: depth={lgb_best["max_depth"]}, '
              f'lr={lgb_best["learning_rate"]:.4f}, n_est={lgb_best["n_estimators"]}')

        # Leave-Eastern-Cape-Out CV (realistic estimate)
        print('  [e] Leave-Eastern-Cape-Out CV...')
        ec_mask = (
        ...
        if ec_mask.sum() > 10:
            ...
            m_eco = xgb.XGBRegressor(**xgb_best, random_state=42, n_jobs=-1, verbosity=0)
            m_eco.fit(X_train_sel[ec_train_idx], y_train[ec_train_idx])
            pred_eco = m_eco.predict(X_train_sel[ec_val_idx])
            eco_r2 = r2_score(y_train[ec_val_idx], pred_eco)
            print(f'     Leave-EC-Out R²: {eco_r2:.4f} (n_ec={ec_mask.sum()})')
        else:
            print(f'     Skip: only {ec_mask.sum()} EC points')

        # Stacking
        print('  [f] Training stacking ensemble (5 models x 3 seeds)...')
        stacking_pred, avg_pred, weighted_pred, meta_coefs = train_stacking_ensemble(
        ...
        # IDW prediction
        idw_cols = [c for c in X_val_raw.columns if c.startswith('idw_k')]
        if 'idw_k30_p10' in X_val_raw.columns:
            idw_pred = X_val_raw['idw_k30_p10'].values
        elif 'idw_k30_p15' in X_val_raw.columns:
            idw_pred = X_val_raw['idw_k30_p15'].values
        else:
            idw_pred = avg_final

        # Environmental IDW
        if 'env_idw_k30' in X_val_raw.columns:
            env_idw_pred = X_val_raw['env_idw_k30'].values
        else:
            env_idw_pred = idw_pred

        # Combined IDW
        if 'combined_idw_a5' in X_val_raw.columns:
            combined_idw_pred = X_val_raw['combined_idw_a5'].values
        else:
            combined_idw_pred = idw_pred
        ...
        all_predictions[target] = {
            ...
            'idw_geo': idw_pred,
            'idw_env': env_idw_pred,
            'idw_combined': combined_idw_pred,
        }

        print(f'\n  Results for {target}:')
        for name, pred in all_predictions[target].items():
    ...
    # =========================================================================
    print(f'\n{"="*70}')
    print('[6/6] Creating submissions...')
    print(f'{"="*70}')

    submission_configs = [
        # (name, ML_type, ML_weight, IDW_type, IDW_weight)
        ('stacking_pure', 'stacking', 1.0, None, 0.0),
        ('weighted_pure', 'weighted', 1.0, None, 0.0),
        ('avg_pure', 'avg', 1.0, None, 0.0),
        ('stacking_70_geo_idw_30', 'stacking', 0.7, 'idw_geo', 0.3),
        ('stacking_50_geo_idw_50', 'stacking', 0.5, 'idw_geo', 0.5),
        ('stacking_70_env_idw_30', 'stacking', 0.7, 'idw_env', 0.3),
        ('stacking_50_env_idw_50', 'stacking', 0.5, 'idw_env', 0.5),
        ('stacking_70_comb_idw_30', 'stacking', 0.7, 'idw_combined', 0.3),
        ('stacking_50_comb_idw_50', 'stacking', 0.5, 'idw_combined', 0.5),
        ('weighted_70_geo_idw_30', 'weighted', 0.7, 'idw_geo', 0.3),
        ('weighted_50_geo_idw_50', 'weighted', 0.5, 'idw_geo', 0.5),
        ('stacking_40_geo_idw_60', 'stacking', 0.4, 'idw_geo', 0.6),
        ('geo_idw_pure', None, 0.0, 'idw_geo', 1.0),
        ('env_idw_pure', None, 0.0, 'idw_env', 1.0),
        ('combined_idw_pure', None, 0.0, 'idw_combined', 1.0),
    ]

    for name, ml_type, ml_w, idw_type, idw_w in submission_configs:
        sub = pd.DataFrame({
        ...
        for target in TARGET_COLS:
            ...
            if ml_w > 0 and ml_type:
                ml_pred = preds[ml_type]
            else:
                ml_pred = np.zeros(len(sub_template))

            if idw_w > 0 and idw_type:
                idw_pred = preds[idw_type]
            else:
                idw_pred = np.zeros(len(sub_template))

            blended = ml_w * ml_pred + idw_w * idw_pred
            blended = np.clip(blended, cfg['clip'][0], cfg['clip'][1])
            sub[target] = blended

        fname = f'submission_v21_{name}.csv'
        sub.to_csv(os.path.join(BASE_DIR, fname), index=False)
        print(f'  {fname}')

    # =========================================================================
    # Best-of-breed: per-target optimal blend
    # =========================================================================
    # Use stacking for TA and EC (higher CV), weighted for DRP
    print('\n  Creating best-of-breed submission...')
    ...
    # For each target, blend stacking with combined IDW
    target_blend_weights = {
        'Total Alkalinity': (0.6, 0.4),       # ML, IDW
        'Electrical Conductance': (0.6, 0.4),  # ML, IDW
        'Dissolved Reactive Phosphorus': (0.5, 0.5),  # More IDW for harder target
    }

    for target in TARGET_COLS:
        cfg = target_configs[target]
        preds = all_predictions[target]
        ml_w, idw_w = target_blend_weights[target]

        # Use weighted average of stacking and avg as ML
        ml_pred = 0.6 * preds['stacking'] + 0.4 * preds['weighted']
        # Use combined IDW (geo + env)
        idw_pred = preds['idw_combined']

        blended = ml_w * ml_pred + idw_w * idw_pred
        blended = np.clip(blended, cfg['clip'][0], cfg['clip'][1])
        sub_best[target] = blended

    sub_best.to_csv(os.path.join(BASE_DIR, 'submission_v21_best.csv'), index=False)
    print('  submission_v21_best.csv (per-target optimal blend)')
    for target in TARGET_COLS:
        print(f'    {target}: mean={sub_best[target].mean():.2f}, std={sub_best[target].std():.2f}')

    print(f'\n{"="*70}')
    print('DONE! Generated submissions:')
    print('  RECOMMENDED: submission_v21_best.csv')
    print('  ALTERNATIVES: submission_v21_stacking_pure.csv,')
    print('                submission_v21_stacking_70_comb_idw_30.csv')
    print(f'{"="*70}')
